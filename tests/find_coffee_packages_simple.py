...
# -*- coding: utf-8 -*-
"""
Упрощенный поиск пачек кофе из группы Coffee Blasercafe
"""
...
def find_coffee_packages_simple():
    """Упрощенный поиск пачек кофе"""
    try:
        with DatabaseConnector() as db:
            print("=== ПОИСК ПАЧЕК КОФЕ ===")
            
            # 1. Находим группы кофе
            query1 = """
            SELECT ID, NAME
            FROM goodsgroups 
...
def main():
    """Основная функция"""
    print("=== ПОИСК ПАЧЕК КОФЕ ИЗ ГРУПП КОФЕ ===")
    print(f"Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Этап 1: Находим группы кофе
    print("\n" + "="*50)
    print("ЭТАП 1: Поиск групп кофе")
    print("="*50)
    
    packages = find_coffee_packages_simple()
