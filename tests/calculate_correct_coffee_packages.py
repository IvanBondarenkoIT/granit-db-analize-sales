...
# -*- coding: utf-8 -*-
"""
Правильный расчет килограммов только для пачек кофе
"""
...
def get_coffee_packages_only(start_date, end_date):
    """Получает только пачки кофе (не напитки)"""
    try:
        with DatabaseConnector() as db:
            print(f"=== ПАЧКИ КОФЕ (НЕ НАПИТКИ) ({start_date} - {end_date}) ===")
            
            # Ищем только пачки кофе (исключаем напитки)
            query = """
            SELECT 
...
def main():
    """Основная функция"""
    print("=== ПРАВИЛЬНЫЙ РАСЧЕТ ПАЧЕК КОФЕ ===")
    print(f"Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Этап 1: Получаем только пачки кофе
    print("\n" + "="*50)
    print("ЭТАП 1: Пачки кофе (не напитки)")
    print("="*50)
    
    sales_data = get_coffee_packages_only("2025-09-29", "2025-09-30")
