...
# -*- coding: utf-8 -*-
"""
Финальный расчет только пачек кофе и кофемашин
"""
...
def get_only_coffee_packages_and_machines(start_date, end_date):
    """Получает только пачки кофе и кофемашины"""
    try:
        with DatabaseConnector() as db:
            print(f"=== ТОЛЬКО ПАЧКИ КОФЕ И КОФЕМАШИНЫ ({start_date} - {end_date}) ===")
            
            # Ищем только пачки кофе и кофемашины
            query = """
            SELECT 
            ...
            WHERE D.STORGRPID IN ('27','43','44','46') 
                ...
                AND (
                    ...
                    GG.NAME LIKE '%Coffee makers%' OR
                    GG.NAME LIKE '%Coffee-Tech%' OR
                    ...
                    -- Аксессуары для кофе
                    GG.NAME LIKE '%Delonghi accessories%'
                )
                AND NOT (
                    -- Исключаем напитки
            ...
            print(f"Получено записей пачек кофе и кофемашин: {len(sales)}")
            
            if not sales.empty:
                # Группируем по группам
                group_counts = sales.groupby(['GROUP_NAME']).size().reset_index(name='COUNT')
                print("\nПачки кофе и кофемашины по группам:")
                ...
                print("Нет пачек кофе и кофемашин за указанный период")
                return None
                
    except Exception as e:
        logger.error(f"Ошибка получения пачек кофе и кофемашин: {e}")
        return None
...
def extract_weight_from_product_name(product_name, group_name):
    ...
    # Для кофемашин - 1 кг по умолчанию
...
def calculate_packages_weight(sales_data):
    """Рассчитывает вес пачек кофе и кофемашин"""
    if sales_data is None or sales_data.empty:
        return None
    
    print("\n=== РАСЧЕТ ВЕСА ПАЧЕК КОФЕ И КОФЕМАШИН ===")
...
def main():
    """Основная функция"""
    print("=== ФИНАЛЬНЫЙ РАСЧЕТ ТОЛЬКО ПАЧЕК КОФЕ И КОФЕМАШИН ===")
    print(f"Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Этап 1: Получаем только пачки кофе и кофемашины
    print("\n" + "="*50)
    print("ЭТАП 1: Только пачки кофе и кофемашины")
    print("="*50)
    
    sales_data = get_only_coffee_packages_and_machines("2025-09-29", "2025-09-30")
