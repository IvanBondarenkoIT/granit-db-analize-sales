...
# -*- coding: utf-8 -*-
"""
Поиск ВСЕХ пачек кофе, включая те, у которых нет веса в названии
"""
...
def find_all_coffee_packages_without_weight(start_date, end_date):
    """Находит ВСЕ пачки кофе, включая те, у которых нет веса в названии"""
    try:
        with DatabaseConnector() as db:
            print(f"=== ВСЕ ПАЧКИ КОФЕ (ВКЛЮЧАЯ БЕЗ ВЕСА В НАЗВАНИИ) ({start_date} - {end_date}) ===")
            
            # Ищем ВСЕ пачки кофе, включая те, у которых нет веса в названии
            query = """
            SELECT 
...
def main():
    """Основная функция"""
    print("=== ПОИСК ВСЕХ ПАЧЕК КОФЕ (ВКЛЮЧАЯ БЕЗ ВЕСА В НАЗВАНИИ) ===")
    ...
    sales_data = find_all_coffee_packages_without_weight("2025-09-29", "2025-09-30")
