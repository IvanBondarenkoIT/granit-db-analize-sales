...
# -*- coding: utf-8 -*-
"""
Финальное тестирование продаж пачек кофе
"""
...
def get_coffee_packages_sales(start_date, end_date):
    """Получает продажи пачек кофе из групп кофе"""
    try:
        with DatabaseConnector() as db:
            print(f"=== ПРОДАЖИ ПАЧЕК КОФЕ ({start_date} - {end_date}) ===")
            
            # Ищем товары из групп кофе (пачки, не напитки)
            query = """
            SELECT 
            ...
            WHERE D.STORGRPID IN ('27','43','44','46') 
                ...
                AND (
                    GG.NAME LIKE '%Coffee Blasercafe blend (1kg)%' OR
                    GG.NAME LIKE '%Coffee Blasecafe blend (250 g)%' OR
                    GG.NAME LIKE '%Coffee Blasercafe singl origin (250g)%' OR
                    GG.NAME LIKE '%Coffee Fresh Dim Kava Single origin%' OR
                    GG.NAME LIKE '%Coffee Fresh Orient Roastery 1 kg%' OR
                    GG.NAME LIKE '%Coffee Fresh Orient Roastery 0,250 g%'
                )
...
def main():
    """Основная функция"""
    print("=== ФИНАЛЬНОЕ ТЕСТИРОВАНИЕ ПАЧЕК КОФЕ ===")
    print(f"Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Этап 1: Получаем продажи пачек
    print("\n" + "="*50)
    print("ЭТАП 1: Продажи пачек кофе")
    print("="*50)
    
    sales_data = get_coffee_packages_sales("2025-09-29", "2025-09-30")
