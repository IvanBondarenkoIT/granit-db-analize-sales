...
# -*- coding: utf-8 -*-
"""
Поиск ВСЕХ пачек кофе с правильными весами
"""
...
def find_all_coffee_packages_correct_weights(start_date, end_date):
    """Находит ВСЕ пачки кофе с правильными весами"""
    try:
        with DatabaseConnector() as db:
            print(f"=== ВСЕ ПАЧКИ КОФЕ С ПРАВИЛЬНЫМИ ВЕСАМИ ({start_date} - {end_date}) ===")
...
def extract_weight_from_package_name(product_name):
    """Извлекает вес из названия пачки кофе с правильными весами"""
    import re
...
def calculate_packages_weight_detailed(sales_data):
    """Рассчитывает вес пачек кофе с детальным анализом"""
    if sales_data is None or sales_data.empty:
        return None
    
    print("\n=== ДЕТАЛЬНЫЙ РАСЧЕТ ВЕСА ПАЧЕК КОФЕ ===")
    ...
    # Анализируем веса
    print("\nАнализ весов пачек кофе:")
    weight_analysis = sales_data.groupby('ITEM_WEIGHT').agg({
        'QUANTITY': 'sum',
        'TOTAL_WEIGHT': 'sum',
        'PRODUCT_NAME': 'count'
    }).reset_index()
    weight_analysis.columns = ['WEIGHT_KG', 'TOTAL_QUANTITY', 'TOTAL_WEIGHT_KG', 'UNIQUE_PRODUCTS']
    
    for idx, row in weight_analysis.iterrows():
        print(f"  {row['WEIGHT_KG']} кг: {row['UNIQUE_PRODUCTS']} товаров, {row['TOTAL_QUANTITY']} пачек, {row['TOTAL_WEIGHT_KG']:.4f} кг")
    ...
    print("\nИтоговые килограммы пачек кофе по магазинам:")
    print(grouped)
...
def main():
    """Основная функция"""
    print("=== ПОИСК ВСЕХ ПАЧЕК КОФЕ С ПРАВИЛЬНЫМИ ВЕСАМИ ===")
    print(f"Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Этап 1: Находим все пачки кофе
    print("\n" + "="*50)
    print("ЭТАП 1: Все пачки кофе")
    print("="*50)
    
    sales_data = find_all_coffee_packages_correct_weights("2025-09-29", "2025-09-30")
    ...
    weights_data = calculate_packages_weight_detailed(sales_data)
