...
# -*- coding: utf-8 -*-
"""
Поиск подгрупп кофе с разными весами
"""
...
def find_coffee_subgroups():
    """Находит подгруппы кофе с разными весами"""
    try:
        with DatabaseConnector() as db:
            print("=== ПОИСК ПОДГРУПП КОФЕ ===")
            
            # 1. Находим все группы кофе
            query1 = """
            SELECT ID, NAME
            FROM goodsgroups 
            ...
            # 2. Находим товары из групп кофе
            query2 = """
            SELECT DISTINCT
                G.ID,
                G.NAME,
            ...
            if not products.empty:
                ...
                # Показываем примеры товаров из каждой группы
                print("\nПримеры товаров по группам:")
                for group_name in products['GROUP_NAME'].unique():
                    group_products = products[products['GROUP_NAME'] == group_name]
                    print(f"\n  Группа: {group_name}")
                    for idx, row in group_products.head(5).iterrows():  # Показываем первые 5 товаров
                        print(f"    {row['NAME']}")
...
def main():
    """Основная функция"""
    print("=== ПОИСК ПОДГРУПП КОФЕ С РАЗНЫМИ ВЕСАМИ ===")
    print(f"Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Этап 1: Находим подгруппы кофе
    print("\n" + "="*50)
    print("ЭТАП 1: Поиск подгрупп кофе")
    print("="*50)
    
    packages = find_coffee_subgroups()
