...
# -*- coding: utf-8 -*-
"""
Поиск недостающих товаров кофе для расчета килограммов
"""
...
def find_missing_coffee_products(start_date, end_date):
    """Находит недостающие товары кофе"""
    try:
        with DatabaseConnector() as db:
            print(f"=== ПОИСК НЕДОСТАЮЩИХ ТОВАРОВ КОФЕ ({start_date} - {end_date}) ===")
            
            # Ищем ВСЕ товары кофе, включая те, которые мы могли пропустить
            query = """
            SELECT 
            ...
            print(f"Получено записей товаров кофе: {len(sales)}")
            ...
            if not sales.empty:
                ...
                print("\nПримеры товаров кофе:")
                for idx, row in sales.head(30).iterrows():
...
def extract_weight_from_product_name(product_name, group_name):
    ...
    # Для кофемашин - более реалистичный вес
    if any(keyword in group_name.lower() for keyword in ['makers', 'gaggia', 'macap', 'saeco', 'handpresso', 'melitta', 'delonghi']):
        # Кофемашины обычно весят 3-15 кг
        if 'titanium' in product_name.lower():
            return 15.0  # Титановая кофемашина
        elif 'magnifica' in product_name.lower():
            return 12.0  # Magnifica кофемашина
        else:
            return 5.0   # По умолчанию для кофемашин
    
    # Для аксессуаров
    if 'accessories' in group_name.lower():
        return 0.5  # Аксессуары обычно легкие
    
    # Для пачек кофе - извлекаем вес из названия
    weight_patterns = [
        r'(\d+[,.]?\d*)\s*кг',
        r'(\d+[,.]?\d*)\s*kg',
        r'(\d+[,.]?\d*)\s*г',
...
def calculate_packages_weight(sales_data):
    """Рассчитывает вес товаров кофе"""
    if sales_data is None or sales_data.empty:
        return None
    
    print("\n=== РАСЧЕТ ВЕСА ТОВАРОВ КОФЕ ===")
    
    # Добавляем расчет веса
    sales_data['ITEM_WEIGHT'] = sales_data.apply(
        lambda row: extract_weight_from_product_name(row['PRODUCT_NAME'], row['GROUP_NAME']), 
        axis=1
    )
...
def main():
    """Основная функция"""
    print("=== ПОИСК НЕДОСТАЮЩИХ ТОВАРОВ КОФЕ ===")
    print(f"Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Этап 1: Находим недостающие товары кофе
    print("\n" + "="*50)
    print("ЭТАП 1: Поиск недостающих товаров кофе")
    print("="*50)
    
    sales_data = find_missing_coffee_products("2025-09-29", "2025-09-30")
