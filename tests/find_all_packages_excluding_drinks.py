...
# -*- coding: utf-8 -*-
"""
Поиск ВСЕХ пачек и товаров с весом (исключая напитки)
"""
...
def find_all_packages_excluding_drinks(start_date, end_date):
    """Находит ВСЕ пачки и товары с весом (исключая напитки)"""
    try:
        with DatabaseConnector() as db:
            print(f"=== ВСЕ ПАЧКИ И ТОВАРЫ С ВЕСОМ (ИСКЛЮЧАЯ НАПИТКИ) ({start_date} - {end_date}) ===")
            
            # Ищем ВСЕ товары, исключая напитки
            query = """
            SELECT 
            ...
            WHERE D.STORGRPID IN ('27','43','44','46') 
                ...
                AND NOT (
                    ...
                    GG.NAME LIKE '%Cleaning%' OR
                    G.NAME LIKE '%Espresso%' OR
                    G.NAME LIKE '%Cold%' OR
                    G.NAME LIKE '%Tonic%' OR
                    G.NAME LIKE '%Smoothie%' OR
                    G.NAME LIKE '%Tropic%'
                )
...
def calculate_packages_weight(sales_data):
    """Рассчитывает вес пачек и товаров"""
    if sales_data is None or sales_data.empty:
        return None
    
    print("\n=== РАСЧЕТ ВЕСА ПАЧЕК И ТОВАРОВ ===")
...
def main():
    """Основная функция"""
    print("=== ПОИСК ВСЕХ ПАЧЕК И ТОВАРОВ С ВЕСОМ (ИСКЛЮЧАЯ НАПИТКИ) ===")
    print(f"Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Этап 1: Находим все пачки и товары с весом
    print("\n" + "="*50)
    print("ЭТАП 1: Все пачки и товары с весом")
    print("="*50)
    
    sales_data = find_all_packages_excluding_drinks("2025-09-29", "2025-09-30")
