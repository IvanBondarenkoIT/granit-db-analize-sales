...
# -*- coding: utf-8 -*-
"""
Расчет веса для ВСЕХ товаров кофе (включая напитки)
"""
...
def get_all_coffee_products(start_date, end_date):
    """Получает ВСЕ товары кофе (включая напитки)"""
    try:
        with DatabaseConnector() as db:
            print(f"=== ВСЕ ТОВАРЫ КОФЕ ({start_date} - {end_date}) ===")
        ...
        logger.error(f"Ошибка получения товаров кофе: {e}")
        return None

def extract_weight_from_product_name(product_name, group_name):
    """Извлекает вес из названия товара кофе"""
    import re
    
    if not product_name:
        return 0.25  # По умолчанию
    
    # Для напитков (espresso, Cold, Tonic, Smoothie) - 0.25 кг за порцию
    if any(keyword in group_name.lower() for keyword in ['espresso', 'cold', 'tonic', 'smoothie', 'tropic']):
        return 0.25
...
def calculate_all_coffee_weight(sales_data):
    ...
    print("Итоговые килограммы всех товаров кофе по магазинам:")
    print(grouped)
...
def main():
    """Основная функция"""
    print("=== РАСЧЕТ ВЕСА ВСЕХ ТОВАРОВ КОФЕ ===")
    print(f"Дата: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Этап 1: Получаем все товары кофе
    print("\n" + "="*50)
    print("ЭТАП 1: Все товары кофе")
    print("="*50)
    
    sales_data = get_all_coffee_products("2025-09-29", "2025-09-30")
    
    # Этап 2: Рассчитываем веса
    print("\n" + "="*50)
    print("ЭТАП 2: Расчет весов всех товаров")
    print("="*50)
    
    weights_data = calculate_all_coffee_weight(sales_data)
